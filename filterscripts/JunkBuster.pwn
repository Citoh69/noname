...
PRIVATE: DisplayKickBanText (playerid, reason [], bool: ban)
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "~g~Si ha sido injustamente %s, Pulse ~b~F8~g~ para hacer una captura de pantalla, reporta en ~r~www.gc-rp.net/foro~g~.", ban ? ("banned") : ("kicked"), JB::Homepage);
...
PRIVATE: ConfigJunkBuster ()
{
	...
	{
	    JB::Homepage = "www.gc-rp.net/foro";
